...
test("backoff dobra com a tentativa e respeita o cap", () => {
...
test("full jitter fica em [0, cap]", () => {
...
test("RetryExecutor retorna sucesso na primeira chamada", async () => {
...
test("RetryExecutor retenta em 429 até conseguir", async () => {
...
test("RetryExecutor respeita retryAfterMs explícito (simula Retry-After)", async () => {
  const delays: number[] = [];
  ...
  const op = async (): Promise<RetryOutcome<unknown>> => {
    n++;
    if (n < 2) return retry("429 com Retry-After=0.05s", { status: 429, retryAfterMs: 50, retryable: true });
    return success(null);
  };
  await exec.run(op);
  const elapsed = Date.now() - start;
  assert.ok(elapsed >= 40, `esperado >=40ms, veio ${elapsed}ms`);
  assert.ok(elapsed < 1000);
  void delays;
});

test("RetryExecutor lança NonRetryableError em 4xx (≠429)", async () => {
  const exec = new RetryExecutor(opts);
...
test("RetryExecutor lança MaxRetriesExceededError ao exceder tentativas", async () => {
  const exec = new RetryExecutor({ ...opts, baseBackoffMs: 1, maxBackoffMs: 2 });
  const op = async (): Promise<RetryOutcome<never>> =>
    retry("sempre 429", { status: 429, retryable: true });
  await assert.rejects(() => exec.run(op), (err: unknown) => {
